...
namespace linkedLists {
    template<typename T>
    class doublycircledList : protected List<T> {
    protected:
        Node<T> *head;
        doublyNode<T> *tail;
    public:
        doublycircledList(doublycircledList<T> const &list) : doublycircledList<T>(list) {
        }

        doublycircledList(T *t) : List<T>(t) {

        }

        doublycircledList(Node<T> *node) : List<T>(node) {

        }

        doublycircledList(int i) : List<T>(i) {
        }

        doublycircledList() : List<T>() {
            head = nullptr;
            tail = nullptr;
        }

        ~doublycircledList() override {
            clear();
        }
